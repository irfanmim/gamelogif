...
:- dynamic(uang/1).
:- dynamic(energi/1).
:- dynamic(ada_orang_di/1).
:- dynamic(tertangkap/1).
:- dynamic(misi/1).
:- retractall(at(_, _)), retractall(aku_di(_)), retractall(alive(_)), retractall(uang(_)), retractall(energi(_)), retractall(ada_orang_di(_,_)), retractall(tertangkap(_)), retractall(misi(_,_)).
...
talk(pedagang) :-
	aku_di(pasar),
	ada_orang_di(copet,markas_copet_pasar),!,
	write('toloong!! saya kecopetan! dia lari ke arah utara').

talk(pedagang) :-
	aku_di(pasar),
	write('terima kasih sudah menangkap pencopetnya').
...
tangkap :-
	...
	write('Selamat kamu berhasil menangkap copet').

tangkap :-
	write('tak ada yg bisa kamu tangkap').
...
ambilmisi(X) :-
	(misi(Y,onprogres) -> write('Kamu sedang dalam misi '),write(Y) ; 
	cekmisi(X)).
